...
def compute_nli_prec_rec_autoais(
    ...
    answer = ''
    for sent in objet_json['answer']:
        text = sent['text'].strip()
        if len(sent['citations'])>0 :
            answer += ' '+ text[:-1] + aux(sent['citations']) + '.'
        else :
            answer += ' '+ text
            
    passages = []
    for docid in objet_json['references']:
        doc = eval(searcher.doc(docid).raw())
        passages.append(doc['segment'])
...
def compute_nli_autoais_dataset(
    ...
    if autoais_model is None:
        ...
        autoais_tokenizer = AutoTokenizer.from_pretrained(AUTOAIS, use_fast=False, legacy=False)
    measure1 = 0
    measure2 = 0

    with open(dataframe, 'r', encoding='utf-8') as fichier:
        for ligne in tqdm(fichier):
            # Convertir chaque ligne en un objet Python (dictionnaire)
            objet_json = json.loads(ligne.strip())
            ...
            if nli_prec_recall:
                ...
                autoais_only_cited_sentences, autoais_all_sentences = compute_nli_autoais(
                    objet_json[column_names[0]],
                    objet_json[column_names[1]],
...
def main():
    ...
    file_result = f"{experiment_folder}/{args.output_file}"

    ############# citations overlap:

    scores = compute_nli_autoais_dataset(
        file_result,
        column_names=["citations", "text"],
        autoais_citation=True,
