...
def deduplicate(doc_list):
    ...
    for p in doc_list:
        if p["docid"] not in set_ids:
            set_candidates.append(p)
            set_ids.append(p["docid"])
...
def sort_all_scores(original_query: str, generated_queries: List[str], ranker):
    ...
    for q in queries:
        ...
        for doc in top_docs:
            docid = doc["docid"]
            score = doc["score"]
            if docid not in best_docs or best_docs[docid]["score"] < score:
                best_docs[docid] = doc

    sorted_best_docs = sorted(best_docs.values(), key=lambda x: x["score"], reverse=True)
    return sorted_best_docs
...
def rerank_against_query(
    ...
    reranked_all_docs = monot5.rerank(original_query, set_candidates, gen = True)

    """ids = [doc["docid"] for doc in reranked_all_docs]
    text = [doc["body"] for doc in reranked_all_docs]
    score = [doc["score"] for doc in reranked_all_docs]"""
    return reranked_all_docs[:100]
...
def vote(
    ...
    for q in queries:
        ...
        top_20 = top_docs[:20]
        for doc in top_20:
            if doc["docid"] in list(scores_by_id.keys()):
                scores_by_id[doc["docid"]].append(doc["score"])
            else:
                scores_by_id[doc["docid"]] = [doc["score"]]
                text_by_ids[doc["docid"]] = doc["text"]
    if score_compute == "count":  # sort by the number of times it was retrieved
        ...
        score = [len(s) for s in list(selected_docs.values())]
    ...
    for i,docid in enumerate(ids) : 
        ...
        docs = eval(searcher.doc(docid).raw())
        result.append({
                        "docid" : docs['docid'] ,
                        "score" : score[i] ,
                        'docs' : {
                            ...
                            "start_char" : docs['start_char'], 
                            "end_char" : docs['end_char']
                        }
                    })
...
def combSum(
    ...
    for q in queries:
        ...
        for doc in top_docs:
            if doc["docid"] in list(scores_by_id.keys()):
                scores_by_id[doc["docid"]].append((doc["score"] - min_val) / range_val)
            else:
                scores_by_id[doc["docid"]] = [(doc["score"] - min_val) / range_val]
                text_by_ids[doc["docid"]] = doc["text"]
