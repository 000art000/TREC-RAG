...
from src.data.data import remove_special_spaces


set_seed(CONFIG['langauge_model']["zephyr"]["SEED"])
...
def generate_queries(
    ...
    if include_answer and answer:
        user_prompt = re.sub(r"\{query\}", query, prompt["user_with_answer"])
        user_prompt = re.sub(r"\{answer\}", answer, user_prompt)
    else:
        query = remove_special_spaces(query)
        user_prompt = re.sub(r"\{query\}", query, prompt['user'])

    system_prompt = re.sub(
        r"\{nb_queries\}", str(nb_queries_to_generate), prompt["system"]
    )
    input_text = [
        {
            "role": "system",
